...
#[cfg(feature = "debug")]
pub struct LogFlushPlugin<S: GetState + Debug>(PhantomData<S>);

#[cfg(feature = "debug")]
impl<S: GetState + Debug> Plugin for LogFlushPlugin<S> {
    fn build(&self, app: &mut App) {
...
#[cfg(feature = "debug")]
impl<S: GetState + Debug> Default for LogFlushPlugin<S> {
    fn default() -> Self {
        Self(PhantomData)
    }
}
